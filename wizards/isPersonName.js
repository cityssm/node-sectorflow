 ...
 */
export async function isPersonName(sectorFlow, name) {
    ...
    const response = await sectorFlow.sendChatMessage(workspace.id, `Is "${name}" a person's name? Respond with either "true" or "false".`);
    await sectorFlow.deleteWorkspace(workspace.id);
    return (response.choices[0].choices[0].message.content.trim().toLowerCase() ===
        'true');
}
